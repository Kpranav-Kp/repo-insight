...
export function CurrentSession({ activeSession }) {
  const [stage, setStage] = useState("idle"); // idle | analyzing | ready | thinking
  ...
  // Load an existing session when the user clicks one in history
  useEffect(() => {
    if (!activeSession) return;
    localIdRef.current = activeSession.localId;
    setRepoUrl(activeSession.repoUrl || "");
  ...
  // Auto-save the session anytime something meaningful changes
  useEffect(() => {
    // don't save the empty welcome screen
